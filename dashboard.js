...
/* Per-item prices used for Beli Nota MDS (nota value calc) were hardcoded above and
   needed a code change + redeploy to revise. ITEM_PRICE_OVERRIDES holds just the
   edited items, persisted to Firestore so anyone with dashboard access can revise a
   price without touching code; loadItemPriceOverrides() merges them onto the
   hardcoded defaults above (mutating the same ITEM_PRICE object in place, so every
   existing reader of ITEM_PRICE keeps working unchanged). */
let ITEM_PRICE_OVERRIDES={};
async function loadItemPriceOverrides(){
  if(!db)return;
  try{
    const snap=await db.collection('app_data').doc('item_price_overrides').get();
    if(snap.exists){
      ITEM_PRICE_OVERRIDES=snap.data().prices||{};
      Object.entries(ITEM_PRICE_OVERRIDES).forEach(([name,v])=>{
        ITEM_PRICE[name]={...(ITEM_PRICE[name]||{}),...v};
      });
    }
  }catch(e){console.warn('loadItemPriceOverrides failed',e);}
  renderItemPriceTable();
  render();
}
function updateItemPrice(name,field,val){
  const n=Math.max(0,Math.round(Number(val)||0));
  if(!ITEM_PRICE[name])ITEM_PRICE[name]={ctn:0,pcs:0};
  ITEM_PRICE[name][field]=n;
  if(!ITEM_PRICE_OVERRIDES[name])ITEM_PRICE_OVERRIDES[name]={};
  ITEM_PRICE_OVERRIDES[name][field]=n;
  if(!db)return;
  db.collection('app_data').doc('item_price_overrides').set({prices:ITEM_PRICE_OVERRIDES,updatedAt:new Date().toISOString()}).catch(e=>console.error('updateItemPrice failed',e.code,e.message));
}
function openItemPriceModal(){
  document.getElementById('item-price-modal').classList.remove('hidden');
  document.getElementById('item-price-search').value='';
  renderItemPriceTable();
}
function renderItemPriceTable(){
  const tb=document.getElementById('item-price-body');
  if(!tb)return;
  const q=(document.getElementById('item-price-search')?.value||'').trim().toLowerCase();
  const names=Object.keys(ITEM_PRICE).sort();
  const filtered=q?names.filter(n=>n.toLowerCase().includes(q)):names;
  tb.innerHTML=filtered.map(n=>{
    const v=ITEM_PRICE[n];
    const edited=ITEM_PRICE_OVERRIDES[n];
    return`<tr>
      <td class="td-main" style="font-size:12px">${n}${edited?' <span style="color:var(--accent);font-size:10px">(direvisi)</span>':''}</td>
      <td><input type="number" class="fi" style="width:110px" value="${v.ctn||0}" onchange="updateItemPrice('${n.replace(/'/g,"\\'")}','ctn',this.value)"></td>
      <td><input type="number" class="fi" style="width:100px" value="${v.pcs||0}" onchange="updateItemPrice('${n.replace(/'/g,"\\'")}','pcs',this.value)"></td>
    </tr>`;
  }).join('')||'<tr><td colspan="3" style="text-align:center;color:var(--t3);padding:24px">Tidak ada item yang cocok.</td></tr>';
  const tf=document.getElementById('item-price-tfoot');
  if(tf)tf.textContent=`${filtered.length} dari ${names.length} item`;
}
...
const WOW_MDS_SHORT_TO_FULL_DS={
  'Gorontalo':{'Apin':'Mohammad Rahman Marwan','Rio':'Satrio Yusuf','Abdul':'Abd. Rahman Lahay','Adit':'Aditya Hulopi'}
};
function wowMdsFullName(t){
  if(!t.mds)return'';
  const shortMap=WOW_MDS_SHORT_TO_FULL_DS[t.area];
  return(shortMap&&shortMap[t.mds])||t.mds;
}
let WU_SORT='area',WU_DIR=1;
function wuSortBy(c){ if(WU_SORT===c)WU_DIR*=-1; else{WU_SORT=c;WU_DIR=1;} renderWowUnvisited(); }
function wuAr(c){ return`<span style="opacity:.25;margin-left:2px;font-size:9px">${WU_SORT===c?(WU_DIR>0?'↑':'↓'):'↕'}</span>`; }
function renderWowUnvisited(){
  ...
  const filtered=q?unvisited.filter(t=>t.name.toLowerCase().includes(q)||(t.pair&&t.pair.toLowerCase().includes(q))||(t.area&&t.area.toLowerCase().includes(q))||wowMdsFullName(t).toLowerCase().includes(q)):unvisited;
  th.innerHTML=`<tr>
    <th style="cursor:pointer" onclick="wuSortBy('area')">Area${wuAr('area')}</th>
    <th style="cursor:pointer" onclick="wuSortBy('name')">Nama Toko${wuAr('name')}</th>
    <th>Toko Pasangan</th>
    <th style="cursor:pointer" onclick="wuSortBy('mds')">MDS${wuAr('mds')}</th>
  </tr>`;
  filtered.sort((a,b)=>{
    let av,bv;
    if(WU_SORT==='mds'){ av=wowMdsFullName(a); bv=wowMdsFullName(b); }
    else if(WU_SORT==='name'){ av=a.name||''; bv=b.name||''; }
    else{ av=a.area||''; bv=b.area||''; }
    const cmp=av.localeCompare(bv)||a.name.localeCompare(b.name);
    return WU_DIR*cmp;
  });
  tb.innerHTML=filtered.length?filtered.map(t=>`<tr><td class="td-dim">${t.area||'—'}</td><td class="td-main">${t.name}</td><td class="td-dim">${t.pair||'—'}</td><td class="td-dim">${wowMdsFullName(t)||'—'}</td></tr>`).join(''):'<tr><td colspan="4" style="text-align:center;color:var(--t3);padding:32px">Semua toko sudah pernah divisit / tidak ada yang cocok pencarian.</td></tr>';
